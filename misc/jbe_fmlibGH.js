  ...
  var fm_menu=
    '<div id="FM_BTNS" data-mode=0 style="width:100%;height:100%;">'+
      ...
      '<div id="FM_CLOSE_BTN" class="fm_class_box" style="float:right;">'+
        //'<div class="class_footer" onclick="JBE_CLOSE_VIEW2()">'+
        '<div class="class_footer" onclick="FM_QUIT()">'+
...
function FM_CHK_REC(recno){  
  recno=recno.toUpperCase();
  let rval=false;
  let fld=FM_FIELDS[0]['fld'];
  for(let i=0;i<FM_TABLE.length;i++){
    let tbl_fld=FM_TABLE[i][fld];
    if(tbl_fld.toUpperCase()==recno){
      rval=true;
      break;
    }
  }
...
async function FM_SAVE_REC(){  
  ...
  let cond='';         //add
  if(!FM_ADD_FLAG){    //edit
    cond=record => !(record[FM_RKEY] === recno);
  }
  let arr=[];
  let ob_items='';
  let len=fldnames.length;
  for(var i=0;i<len;i++){
    ob_items+='"'+fldnames[i]+'":"'+fldvals[i]+'",';
  }        
  ob_items=JSON.parse('{'+ob_items.substring(0,ob_items.length-1)+'}');    
  arr[0]=ob_items;

  await api_save(JBE_CLOUD,JBE_API+FM_TABLE_NAME,arr,cond);  
  let data=await api_readfile(JBE_CLOUD,JBE_API+FM_TABLE_NAME); FM_TABLE=data.content;
  fn(2,FM_TABLE);
  showProgress(false);
  if(FM_FM_MODE==1){ 
    FM_DISP_REC(recno);         
...
function FM_SAVE_REC2(recno){    
  ...
  */
  ...
  .then(async function (response) {
    ...
    axios.get('/api/get_all', { params: {tbl:FM_TABLE_NAME2} })
    .then(function (response) { 
...
//
async function FM_REFRESH_REC(){ 
  return;
  //alert('FM_REFRESH_REC:'+FM_TABLE);
  //get_db_all(FM_TABLE_NAME);
  let data=await api_readfile(JBE_CLOUD,JBE_API+FM_TABLE_NAME); FM_TABLE=data.content;
  if(FM_FM_MODE==2){ let data2=await api_readfile(JBE_CLOUD,JBE_API+FM_TABLE_NAME2); FM_TABLE2=data2.content; }
  snackBar('Refreshed...');
}
...
async function FM_DELREC_ALL() {
  ...
  let data=await api_readfile(JBE_CLOUD,JBE_API+FM_TABLE_NAME); FM_TABLE=data.content;
  var fn = window[FM_FUNC.del];
  if (typeof fn === "function"){ fn(2,FM_TABLE); } 
  showProgress(false);       
  FM_INIT_REC();  
}

//
async function FM_DEL_REC2(recno){  //swu  
  //var recno=document.getElementById('FM_HEAD').getAttribute('data-recno');
  /*
  FM_AXIOS_SQL='DELETE from '+FM_TABLE_NAME2+' WHERE '+FM_RKEY+'=?';
  FM_AXIOS_PARA1=recno;
...
async function FM_REC_LOCK(trans,docno,func_edit2){    
  console.log('inside FM_REC_LOCK');  
...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
function FM_LOOKUP(mode,val_Search,div_Search,DB,arySort,tilt,func,flds,func2) { 
  if(!mode){
